# version 0 uses adaptive kernel size median filter to detect strides
import numpy as np
import pandas as pd
...
for file in sensor_data_files:
    ...
    for i, zv in enumerate(zv_list):
        ...
        if det_list[i] == 'lstm':
            ...
            plt.xlabel('Time')
            plt.ylabel('Zero Velocity')
            plt.savefig(os.path.join(output_dir, f'zv_{det_list[i]}_median_filtered_{base_filename}.png'), dpi=1200, bbox_inches='tight')
